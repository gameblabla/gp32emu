...
static int zip_mkpath(char *path, size_t pos) {
  ...
  for (p = path + len; *p && len < MZ_ZIP_MAX_ARCHIVE_FILENAME_SIZE; p++) {
    ...
    defined(__MINGW32__)
      ...
      /*
       * The emulator only uses this bundled library for archive reads, but
       * ZIP_ENABLE_INFLATE also compiles the optional filesystem extraction
       * helper below.  With _FILE_OFFSET_BITS=64, miniz intentionally selects
       * struct stat64/MZ_FILE_STAT on some libc configurations; calling plain
       * lstat() with that storage can then fail to compile because lstat() may
       * be redirected to a struct stat ABI.  Use miniz's matching stat wrapper
       * here so the structure type and function prototype stay paired.
       *
       * ZIP_HAVE_SYMLINK is disabled for this build, so following symlinks in
       * this unused mkdir helper is acceptable and avoids a non-portable
       * lstat/stat64 mix.
       */
      if (MZ_FILE_STAT(npath, &st) < 0) {
        if (MZ_MKDIR(npath) == -1) {
