...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        "\n"
        "SDL3 interactive GP32 frontend. Headless builds do not depend on SDL.\n"
...
int main(int argc, char **argv) {
    ...
    gp32_video_backend_t *video = gp32_video_sdl3_create(&vopt);
    if (!video) {
    ...
    gp32_input_backend_t *input = gp32_input_sdl3_create(&iopt);
    if (!input) {
    ...
    uint64_t fps_tick = (uint64_t)(last_tick_us / 1000u);
    ...
    while (!quit) {
        ...
        }
        {
            ...
            if (elapsed >= 1000u) {
                ...
                char title[256];
                snprintf(title, sizeof(title), "gp32emu SDL3 - %.1f render / %.1f emu FPS - %s", render_fps, emu_fps, game_label);
